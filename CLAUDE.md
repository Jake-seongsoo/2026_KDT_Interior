...
| AI | Claude Sonnet 4.6 (Vision + Web Search), Vertex AI Imagen 4 |
...
**필수 프롬프트 전략** (`services/claude_service.py` 수정 시 준수):
1. Few-shot 프롬프팅 — 33평형 기준 예시 도면 2~3장 첨부 (정확도 ~109% 향상 사례)
2. 프롬프트 분해 4단계 순서: 공간 인식 → 구조 인식 → 수치 추출 → 문/창문 확인
3. 낮은 신뢰도 항목 → 프론트엔드에서 사용자 확인 요청 UI 표시

Few-shot 예시 도면 후보는 AI Hub 데이터셋 ID 71465 (aihub.or.kr) 또는 LH 공공데이터 (data.go.kr/data/15037046) 활용.
...
렌더링 이미지 하단:
> "이 이미지는 Gemini Imagen(생성형 AI)으로 제작되었습니다"
...
### 개인정보 동의 UI (도면 업로드 직전 최초 1회 모달)

모달에 반드시 포함할 4항목: ① 수집 항목(도면 이미지) ② 수집 목적 ③ 보유 기간(30일) ④ 거부 권리  
1개라도 빠지면 유효한 동의 아님 (개인정보보호법 제15조). 모든 렌더링 이미지에 워터마크 또는 EXIF 메타데이터 삽입 필요.

## 구현 로드맵

### Phase 1 — MVP (완료 목표)
- Claude Vision Few-shot 프롬프트 (33평형 예시 도면 3장)
- 톤 6개 추천 → 선택 → 방별 Imagen 렌더링 → 이케아 상품 링크 end-to-end
- 개인정보 동의 모달 UI
- AI 생성 이미지 고지 문구 + 워터마크

### Phase 2 — 고도화 (2026.05)
- AI Hub 데이터셋(ID 71465) 추가 → Few-shot v2 프롬프트 정확도 재측정
- 예산 추정 기능(fc8): `UNIT_COSTS` 상수 + 지역 보정계수 4단계(서울·수도권·지방광역시·기타)
- 방 정보 수정 UI (방 이름·개수 보정), 톤 재선택, 분석 기록 조회

### Phase 3 — 완성 (2026.06)
- fc10: Vision 정확도 80%+ 달성 후 벽 분류 기능 (SNS 이미지 → 내 방 적용 가능 여부)
- KDT 발표용 Vision R&D 막대 그래프 (AECV-bench 기준값 → Few-shot → 목표치)
