  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit)} className='space-y-5'>
      <div
        ...
        className={cn(
          'group cursor-pointer rounded-lg border border-dashed p-4 transition-all sm:p-5',
          dragOver
            ? 'border-teal-400 bg-teal-50'
            : preview
              ? 'border-teal-300 bg-white'
              : 'border-slate-300 bg-slate-50 hover:border-teal-300 hover:bg-white',
        )}
      >
        <input
          ref={fileInputRef}
        ...
        {preview ? (
          <div className='space-y-3'>
            ...
            <p className='text-xs text-slate-500'>클릭하면 다른 파일로 교체할 수 있습니다.</p>
          </div>
        ) : (
          <div className='flex flex-col items-center justify-center py-8 text-center'>
            <div className='mb-4 flex h-12 w-12 items-center justify-center rounded-lg bg-white text-teal-600 shadow-sm ring-1 ring-slate-200'>
              <UploadCloud className='h-6 w-6' />
            </div>
            <p className='text-sm font-semibold text-slate-900'>
              도면 이미지를 드래그하거나 클릭해서 선택
            </p>
            <p className='mt-2 text-xs text-slate-500'>JPG / PNG, 최대 5MB</p>
      ...
      <div className='space-y-2'>
        <label className='text-sm font-semibold text-slate-800'>
          공급면적 <span className='font-normal text-slate-400'>(평형, 예: 32)</span>
        </label>
        <input
          type='number'
          step='0.5'
          {...register('floorArea', { valueAsNumber: true })}
          className='h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-900 shadow-sm outline-none transition focus:border-teal-400 focus:ring-2 focus:ring-teal-100'
          placeholder='예: 32'
        />
        {errors.floorArea && (
